...
- **>=8192 bytes**: uses a binary tree to quickly find a free segment with enough capacity for the given allocation. A segment can be composed of any number of blocks. Each block has associated information, such as whether is free or occupied, the length of the segment it belongs to and an index pointing to the previous segment.
- **<8192 bytes**: takes several contiguous blocks using the previous algorithm, and initializes a FixedMemoryPool in them with an appropiate block size. It uses a linked list to keep track of pools with available blocks for each size. 
